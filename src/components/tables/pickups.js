...
import PickupHeader from "../tableHeader/pickups";
...
const PickupTable = () => {
     //New = error
    const { pickUps, loading, error, pageLimit, currentPage, paginationMode } = useSelector((state) => state.data);
    //New
    ...
    //New End
    useEffect(() => {
        dispatch(loadUsersStart({start: 0, end: 6, currentPage: 0}));
    }, []);
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure that you wanted to delete user ?")) {
          ...
          setTimeout(() => 
            dispatch(loadUsersStart({start: 0, end: 6, currentPage: 0})),  200
          ); 
        }
    };
    ...
    const renderPagination = () => {
          ...
          return(
            <div className="pagination">
              ...
              <div>
                <button 
                  onClick={() => 
                    dispatch(loadUsersStart({start: 6, end: 12, currentPage: 1}))
                  }>
                    <CaretRight size={20} />
                  </button>
              </div>
        ...
        } else if(currentPage < pageLimit -1 && pickUps.length === pageLimit){
          return (
            <div className="pagination">
              <div>
                <button
                ...
                pickUps.map((item, index) => (
                  <tr>
                    ...
                    <td>
                      ...
                      <Link to={`/edit-pickup/${item.id}`} className="action edit">
