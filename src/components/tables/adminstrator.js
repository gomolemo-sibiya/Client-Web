...
import Header from "../tableHeader/admin";

import { 
  deleteAdminUserStart,
  filterAdminUserStart,
  loadAdminUserStart,
  sortAdminUserStart
} from "../../redux/action/admin_actions";
...
const Administrator = () => {
    const { adminUsers, loading, error, pageLimit, currentPage, paginationMode } = useSelector((state) => state.data);
    ...
    //New End
    useEffect(() => {
        dispatch(loadAdminUserStart({start: 0, end: 6, currentPage: 0}));
    }, []);
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure that you wanted to delete user ?")) {
          dispatch(deleteAdminUserStart(id));
          toast.success("User Delete Successfully");
          setTimeout(() => 
            dispatch(loadAdminUserStart({start: 0, end: 6, currentPage: 0})),  200
          ); 
        }
    };

    const onFilterChange = (value) => {
        dispatch(filterAdminUserStart(value));
    }
    ...
    const onSortChange = (e) => {
        ...
        if (sortOption.includes(sortValue)){
          ...
          dispatch(sortAdminUserStart(e.target.value));
        } else {
          dispatch(loadAdminUserStart());
          setSortValue("");
        }
    }
    ...
    const renderPagination = () => {
          ...
          return(
            <div className="pagination">
              ...
              <div>
                <button 
                  onClick={() => 
                    dispatch(loadAdminUserStart({start: 6, end: 12, currentPage: 1}))
                  }>
                    <CaretRight size={20} />
                  </button>
              </div>
        ...
        } else if(currentPage < pageLimit -1 && adminUsers.length === pageLimit){
          return (
            <div className="pagination">
              <div>
                <button
                    ...
                    adminUsers.map((item, index) => (
                      <tr>
                        ...
                        <td>
                          ...
                          <Link to={`/edit-admin/${item.id}`} className="action edit">
