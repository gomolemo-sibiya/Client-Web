...
import { createCleanerUserStart, updateCleanerUserStart } from "../../redux/action/cleaner_actions";
...
const AddEditCleaner = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (name && email && phone && address && status) {
      if (!editMode) {
        dispatch(createCleanerUserStart(formValue));
        toast.success("User Added Successfully");
        setTimeout(() => navigate("/cleaners"), 500);
      } else {
        dispatch(updateCleanerUserStart({ id, formValue }));
        setEditMode(false);
        ...
        <MDBValidation noValidate onSubmit={handleSubmit} style={{ display: 'block'}}>
            <Wrapper>
                ...
                <Submit>
                    ...
                    <MDBBtn className="backBtn" onClick={() => navigate("/cleaners")} color="danger">
                        Go Back
                    </MDBBtn>
                </Submit>
            </Wrapper>
