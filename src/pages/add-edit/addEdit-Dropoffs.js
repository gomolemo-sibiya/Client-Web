...
import { createDropOffStart, updateDropOffStart } from "../../redux/action/dropoff_actions";
import { 
    Container,
    Heading,
    Wrapper,
...
const AddEditDropOff = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (name && email && phone && address && status) {
      if (!editMode) {
        dispatch(createDropOffStart(formValue));
        toast.success("User Added Successfully");
        setTimeout(() => navigate("/recycling-dropoffs"), 500);
      } else {
        dispatch(updateDropOffStart({ id, formValue }));
        setEditMode(false);
        ...
        <MDBValidation noValidate onSubmit={handleSubmit} style={{ display: 'block'}}>
            <Wrapper>
                ...
                <Submit>
                    ...
                    <MDBBtn className="backBtn" onClick={() => navigate("/recycling-dropoffs")} color="danger">
                        Go Back
                    </MDBBtn>
                </Submit>
            </Wrapper>
