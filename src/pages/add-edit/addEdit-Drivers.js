...
import { createDriverUserStart, updateDriverUserStart } from "../../redux/action/driver_actions";
import { 
    Container,
    Heading,
    Wrapper,
...
const AddEditDriver = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (name && email && phone && address && status) {
      if (!editMode) {
        dispatch(createDriverUserStart(formValue));
        toast.success("User Added Successfully");
        setTimeout(() => navigate("/drivers"), 500);
      } else {
        dispatch(updateDriverUserStart({ id, formValue }));
        setEditMode(false);
        ...
        <MDBValidation noValidate onSubmit={handleSubmit} style={{ display: 'block'}}>
            <Wrapper>
                ...
                <Submit>
                    ...
                    <MDBBtn className="backBtn" onClick={() => navigate("/drivers")} color="danger">
                        Go Back
                    </MDBBtn>
                </Submit>
            </Wrapper>
